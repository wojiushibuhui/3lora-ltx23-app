...
# 检查 ComfyUI 目录是否存在
if [ ! -d "$COMFY_DIR" ]; then
    log_error "ComfyUI 目录不存在: $COMFY_DIR"
    exit 1
fi
...
if [ -f "$UPDATE_SYMLINKS_SCRIPT" ]; then
    ...
    # 过滤掉 libtinfo 版本警告信息
    bash "$UPDATE_SYMLINKS_SCRIPT" 2>&1 | grep -v "no version information available" || true
    ...
    log_info "模型符号链接更新脚本不存在，跳过: $UPDATE_SYMLINKS_SCRIPT"
fi
...
LOG_FILE="${COMFY_DIR}/user/comfyui-startup.log"
...
set +e
"$PYTHON_BIN" main.py --port $PORT --listen 127.0.0.1 --enable-cors-header "*" 2>&1 | tee -a "$LOG_FILE"
